...
use crate::application::{
    auth_handler::AuthHandler, forum_handler::ForumHandler,
};
use crate::domain::auth_model::AuthError;
use crate::infrastructure::auth::extract_id;
use crate::domain::users_actor::{UsersActor, Broadcast};
               ...
               -> impl Responder
{
    if let Some(user_id) = extract_id(&req) {
        match auth_handler.get_user(user_id).await {
            Ok(user) => {
                match forum_cmd.into_inner() {

                    ForumCmd::MakePost(post_contents) => {

                        let result = forum_handler.make_post(user, &post_contents)
                            .await;
                    ...
                    ForumCmd::FetchPostsBy { start_id, end_id } => {
                        let result = forum_handler.fetch_posts_by(
                    ...
                    ForumCmd::DeletePost { post_id } => {
                        ...
                        HttpResponse::Ok().json(result)
                        } else {HttpResponse::Unauthorized().body("Not admin!")}
                    }
                }
            },
